...
train_dataset=tf.data.Dataset.from_tensor_slices((X_train,y_train)).map(md.load_one_image,num_parallel_calls=AUTOTUNE).cache().batch(batch_size=32).prefetch(AUTOTUNE)
test_dataset=tf.data.Dataset.from_tensor_slices((X_test,y_test)).map(md.load_one_image,num_parallel_calls=AUTOTUNE).cache().batch(batch_size=32).prefetch(AUTOTUNE)
